...
import AddCaseSize from './AddCaseSize'
import { DeleteCaseSize, GetCaseSize } from '../../../../services/caseSizeService'
...
function CaseSizeListing() {
  ...
  const getCaseSizeData = async () => {
    ...
    const response = await GetCaseSize(queryString)
  ...
  const handleDeleteCaseSize = async () => {
    dispatch(showLoader())
    const response = await DeleteCaseSize(selectedItem)
    if (response?.status === 204) {
      deleteToastFun('Case size deleted successfully', 'success')
      handleCloseDeleteModal()
      if (totalItemsOnPage === 1 && itemOffset > 0) {
        setItemOffset(itemOffset - itemsPerPage) // Go to the previous page
      } else {
        getCaseSizeData() // Refresh the data
      }
    } else {
  ...
  const handleApplyAction = () => {
    if (selectedAction) {
      switch (selectedAction) {
        case 'active':
          const activeBody = {
            model: BulkActionModel?.case_size,
            ids: selectedIds,
    ...
    <>
      <WhiteCard>
        <div className='grid grid-cols-12 gap-4'>
          <BulkHeader
            addName='Add Case Size'
            handleAddFun={handleAddCaseSize}
      ...
      {openModal && (
        <Modal
          ...
          header
          headingText={selectedItem?.id ? 'Edit Case Size' : 'Add Case Size'}
          width={'md:w-[700px]'}
        >
          <AddCaseSize
            handleCloseModal={handleCloseModal}
            selectedItem={selectedItem}
            getCaseSizeData={getCaseSizeData}
          />
        </Modal>
      )}
      {openDeleteModal && (
        ...
        >
          <AdminDeleteModal
            ...
            deleteFunction={handleDeleteCaseSize}
            deleteLabel={'Are you sure you want to delete this case size?'}
          />
        </Modal>
      )}
    </>
...
export default CaseSizeListing
