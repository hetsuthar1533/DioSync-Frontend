import { Form, Formik } from 'formik'
import React, { useState, useEffect } from 'react'
import FormLabel from '../../../components/core/typography/FormLabel'
import InputType from '../../../components/core/formComponents/InputType'
import Button from '../../../components/core/formComponents/Button'
import SelectType from '../../../components/core/formComponents/SelectType'
import { ingredientValidationSchema } from '../../../validations/admin/ingredientValidationSchema'
import { AddIngridentsApi, UpdateIngridents } from '../../../services/ingredientService'
import SwitchToggle from '../../../components/core/formComponents/SwitchToggle'
import { GetAllCategories } from '../../../services/categoryService'

function AddIngredient({ selectedItem, getIngredientsData, handleCloseModal }) {
  const [categoriesList, setCategoriesList] = useState([])

  const [defaultInitialValues, setDefaultInitialValues] = useState({
    category: '',
    name: '',
    price: '',
    is_active: true,
  })
  useEffect(() => {
    if (selectedItem) {
      setDefaultInitialValues({
        name: selectedItem.name,
        category: selectedItem.category?.id,
        price: selectedItem.price,
        is_active: selectedItem.is_active,
      })
    }
  }, [])

  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await GetAllCategories()
        if (response?.data?.status === 200) {
          const formattedData = response?.data?.data.map((category) => ({
            label: category.name,
            value: category.id,
          }))
          setCategoriesList(formattedData)
        }
      } catch (error) {
        console.error('Error fetching categories:', error)
      }
    }

    fetchCategories()
  }, [])

  const handleCallListApi = () => {
    handleCloseModal()
    getIngredientsData()
  }

  const OnSubmit = async (data) => {
    if (selectedItem?.id) {
      const response = await UpdateIngridents(data, selectedItem?.id)
      if (response?.status === 200) {
        handleCallListApi()
      }
    } else {
      const response = await AddIngridentsApi(data)
      if (response?.status === 200) {
        handleCallListApi()
      }
    }
  }

  return (
    <Formik
      enableReinitialize
      initialValues={defaultInitialValues}
      validationSchema={ingredientValidationSchema}
      onSubmit={OnSubmit}
    >
      {({ isSubmitting, handleBlur, setFieldValue, values, errors }) => (
        <Form className='grid grid-cols-12 gap-4'>
          <div className='md:col-span-6 col-span-12'>
            <FormLabel>Ingredient name</FormLabel>
            <InputType placeholder='Ingredient name' type='text' name='name' onBlur={handleBlur} />
          </div>
          <div className='md:col-span-6 col-span-12'>
            <FormLabel>Category</FormLabel>
            <SelectType
              fullWidth={'sm:!w-auto !w-full'}
              options={categoriesList}
              placeholder={'Select'}
              error={errors?.category}
              onChange={(option) => setFieldValue('category', option?.value)}
              value={categoriesList?.find((option) => option?.value === values?.category) || ''}
            />
          </div>
          <div className=' md:col-span-6 col-span-12'>
            <FormLabel>Price</FormLabel>
            <InputType placeholder='Price' type='number' name='price' onBlur={handleBlur} />
          </div>
          <div className='md:col-span-6 col-span-12'>
            <FormLabel>Activate/deactivate</FormLabel>
            <SwitchToggle onChange={(value) => setFieldValue('is_active', value)} isChecked={values?.is_active} />
          </div>
          <div className='col-span-12 text-end'>
            <Button primary type='submit' disabled={isSubmitting}>
              {selectedItem?.id ? 'Edit' : 'Save'}
            </Button>
          </div>
        </Form>
      )}
    </Formik>
  )
}

export default AddIngredient
